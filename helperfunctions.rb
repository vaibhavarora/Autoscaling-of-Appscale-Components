...
MAX_VM_CREATION_TIME = 2700 # 10gb image should take ~30 min max if not cached
SLEEP_TIME = 20
IP_REGEX = /\d+\.\d+\.\d+\.\d+/
FQDN_REGEX = /[\w\d\.\-]+/
...
module HelperFunctions
  ...
  #Stops app running on a particular port
  def self.stop_app(app_name, port)
    watch = "appscale-" + app_name + "-" + port.to_s
    GodInterface.stop(watch)
    Djinn.log_debug("Stopped #{watch} task in GOD.")
    GodInterface.remove(watch)
    Djinn.log_debug("Removed #{watch} from GOD.")
  end
  ...
  def self.spawn_vms(num_of_vms_to_spawn, job, image_id, instance_type, keyname,
    ...
    public_up_already = []
    private_up_already = []
    Djinn.log_debug("[#{num_of_vms_to_spawn}] [#{job}] [#{image_id}]  [#{instance_type}] [#{keyname}] [#{infrastructure}] [#{cloud}] [#{group}]")
    ...
    command_to_run = "#{infrastructure}-run-instances #{image_id} -k #{keyname} -n #{num_of_vms_to_spawn} --instance-type #{instance_type} --group #{group}"
